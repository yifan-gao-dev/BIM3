...
from ts_benchmark.baselines.bim3.utils.tools import EarlyStopping, adjust_learning_rate
...
from ts_benchmark.baselines.bim3.models.BIM3_model import BIM3_model
...
class BIM3(ModelBase):
    def __init__(self, **kwargs):
        super(BIM3, self).__init__()
    ...
    def validate(self, valid_data_loader, criterion):
        ...
        with torch.no_grad():
            for input_data, target, input_mark, target_mark in valid_data_loader:
                input_data, target, input_mark, target_mark = (
                ...
                output, _, _ = self.model(input_data, input_mark, target_mark)
    ...
    def forecast_fit(self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float) -> "ModelBase":
        ...
        self.model = BIM3_model(self.config)
        ...
        for epoch in range(config.num_epochs):
            self.model.train()
            # for input_data, target, input_mark, target_mark in train_data_loader:
            for i, (input_data, target, input_mark, target_mark) in enumerate(
                    train_data_loader
            ):
                ...
                # decoder input_data
                output, loss_importance, loss_constraint = self.model(input_data, input_mark, target_mark)
                ...
                torch.cuda.empty_cache()
                total_loss = loss + loss_importance + loss_constraint
                total_loss.backward()
    ...
    def forecast(self, horizon: int, train: pd.DataFrame) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            while answer is None or answer.shape[0] < horizon:
                for input_data, target, input_mark, target_mark in test_data_loader:
                    input_data, target, input_mark, target_mark = (
                        input_data.to(device),
                        target.to(device),
                        input_mark.to(device),
    ...
    def _perform_rolling_predictions(
        ...
        with torch.no_grad():
            ...
            while not answers or sum(a.shape[1] for a in answers) < horizon:
                input_data, dec_input, input_mark, target_mark = (
