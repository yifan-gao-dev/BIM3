...
def parse_arg():
    ...
    # script name
    parser.add_argument("--config-path", type=str, required=True, help="Evaluation config file path")
    parser.add_argument("--data-name-list", type=str, nargs="+", default=None, help="List of series names entered by the user")
    parser.add_argument("--data-set-name", type=str, nargs="+", default=None, help="List of dataset name names entered by the user, only takes effect when data_name_list is not specified")

    # model_config
    parser.add_argument("--adapter", type=str, nargs="+", default=None, help="Adapter used to adapt the method to our pipeline")
    parser.add_argument("--model-name", type=str, nargs="+", required=True, help="The relative path of the model that needs to be evaluated",)
    parser.add_argument("--model-hyper-params", type=str, nargs="+", default=None, help=("The input parameters corresponding to the models to be evaluated, should correspond one-to-one with the --model-name options."))

    # evaluation_config
    parser.add_argument("--metrics", type=str, nargs="+", default=None, help="Evaluation metrics that need to be calculated")
    parser.add_argument("--strategy-args", type=str, default=None, help="Parameters required for evaluating strategies")
    ...
    parser.add_argument("--deterministic", type=str, default="full", choices=["full", "efficient", "none"], help="Specify the type of deterministic behavior for the algorithm. Options are: 'full': Enables full deterministic mode. 'efficient': Fixes only some seeds for efficiency. 'none': No deterministic behavior is applied.")

    # evaluation engine
    parser.add_argument("--eval-backend", type=str, default="sequential", choices=["sequential", "ray"],help="Evaluation backend, use ray for parallel evaluation")
    parser.add_argument("--num-cpus", type=int, default=os.cpu_count(), help="Number of cpus to use, only available in both backends")
    parser.add_argument("--gpus", type=int, nargs="+", default=None, help="List of gpu devices to use, only available in ray backends")
    parser.add_argument("--num-workers", type=int, default=os.cpu_count(), help="Number of evaluation workers")

    # report_config
    parser.add_argument("--aggregate_type", default="mean", help="Select the baseline algorithm to compare")
    parser.add_argument("--save-path", type=str, default=None, help="The relative path for saving evaluation results, relative to the result folder")
    parser.add_argument("--save-true-pred", type=str_to_bool, default=None, help="If true, saves the model's prediction results and the true values in evaluation result file")
...
if __name__ == "__main__":
    ...
    report(report_config, report_method="csv")
